...
func TestHandleCreatePath_Conflict(t *testing.T) {
	...
	createReq2 := httptest.NewRequest("POST", "/api/servers/"+serverID+"/paths", mustJSON(t, map[string]string{
		"user_id": viewerID,
		"path":    "/var/log",
	}))
...
func TestHandleDeletePath_AlreadyDeleted(t *testing.T) {
	...
	perm, err := s.store.CreatePermission(viewerID, serverID, "/var/log")
	if err != nil {
