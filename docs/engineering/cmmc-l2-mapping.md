...
| 3.1.5 | Employ principle of least privilege | Viewers default to zero access - explicit path grants required. Admins have full access (no sub-scoping). Rate limiting on auth endpoints (10/min per IP). | Partially Addressed |
...
| 3.1.8 | Limit unsuccessful logon attempts | Rate limiter: 10 attempts per IP per 60-second sliding window on login, TOTP, and registration endpoints. Returns 429 with `Retry-After: 60` header. | Fully Addressed |
